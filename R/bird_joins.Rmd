...
```{r}
# Read in the data 
taxalist_observations <- read.csv(here("data", "CopyOf52_pp52_taxalist_1.csv"))

surveys_observations <- read.csv(here("data", "CopyOf52_pp52_surveys_1.csv"))

sites_observations <- read.csv(here("data", "CopyOf52_pp52_sites_1.csv"))

bird_observations <- read.csv(here("data", "CopyOf52_pp52_birds_1.csv"))
...
```{r}
birds_left <- left_join(birds_subset, sites_observations, by = "site_id") %>% 
  left_join(taxalist_observations, by = "species_id" )
...
```{r}
birds_full <- full_join(birds_subset, sites_observations, by = "site_id")
```
Inner join practice
...
```{r}
birds_inner <- inner_join(birds_subset, taxalist_observations, by = "species_id")
```
...
bird_observations_new <- bird_observations %>%
  left_join(taxalist_observations, by = "species_id") %>%
  left_join(sites_observations) %>% 
  left_join(surveys_observations)

bird_observations_subset <- bird_observations_new %>% 
...
```{r}
bird_obs_new <- bird_observations_subset %>%
  mutate(survey_month = lubridate::month(survey_date)) %>% 
  dplyr::relocate(survey_month)
```
